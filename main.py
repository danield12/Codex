...
def main():
    ...
    for entry in dk_data:
        ...
        if key not in consolidated:
            consolidated[key] = {'DraftKings': {}, 'Caesars': {}, 'FanDuel': {}, 'Pin': {}}

        consolidated[key]['DraftKings'] = odds

        # Add pin data if available
        if 'pin_x' in entry:
            consolidated[key]['Pin'] = {'x': entry['pin_x'], 'y': entry['pin_y']}
    ...
    print(f"{'Player':<20} | {'Hole':<4} | {'Rnd':<3} | {'Market':<10} | {'DK':<6} | {'CZR':<6} | {'FD':<6} | {'Pin X':<6} | {'Pin Y':<6}")
    print("-" * 105)
    ...
    for key in sorted_keys:
        ...
        pin_x = book_odds.get('Pin', {}).get('x', "-")
        pin_y = book_odds.get('Pin', {}).get('y', "-")
        if pin_x != "-": pin_x = f"{pin_x:.3f}"
        if pin_y != "-": pin_y = f"{pin_y:.3f}"
        ...
        for market_key, market_display in markets:
            ...
            print(f"{player:<20} | {hole:<4} | {round_num:<3} | {market_display:<10} | {dk_val:<6} | {czr_val:<6} | {fd_val:<6} | {pin_x:<6} | {pin_y:<6}")
    ...
    if args.output:
        try:
            with open(args.output, 'w', newline='', encoding='utf-8') as csvfile:
                fieldnames = ["Player", "Hole", "Round", "Market", "DraftKings Odds", "Caesars Odds", "FanDuel Odds", "Pin X", "Pin Y"]
